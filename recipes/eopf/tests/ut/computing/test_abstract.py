...
class TestProcessingUnit(EOProcessingUnit):
    __test__ = False
...
@pytest.mark.parametrize(
    "arrays, processing_unit_type, kwargs, expected_id",
    [
        (
...
def test_abstract_processing_unit(
        ...
        processing_unit_type: Type[EOProcessingStep],
        kwargs,
        expected_id,
):
    ...
    with mock.patch(
        ...
        result = processing_unit.run_validating(*arrays, **kwargs)
        assert result is not None
    mock_run.assert_called_once_with(*arrays, **kwargs)
    assert all(isinstance(i, np.ndarray) for i in mock_run.call_args.args)
