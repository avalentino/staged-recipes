...
def input_dir(tmp_path: Path) -> str:
    ...
    for x_shift in [0, 1_050]:
        for y_shift in [0, 900]:
            ...
            dt.to_zarr(path / f"tile_{counter:02d}.zarr", consolidated=False)
    return str(path)
...
def test_merge_attrs_updated(dt_to_update: DataTree) -> None:
    ...
    assert stac_discovery["id"] == "S02MSIL1C_20210101T010101_00000_B0128_TD59"
...
def test_merge_to_zarr(tmp_path: Path, dt: DataTree) -> None:
    ...
    dt.to_zarr(path, consolidated = False)
    open_datatree(path, engine="zarr", consolidated = False)
