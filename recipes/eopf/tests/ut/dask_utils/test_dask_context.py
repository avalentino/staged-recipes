...
from eopf.common.file_utils import AnyPath  # noqa: E402
from eopf.config.config import EOConfiguration  # noqa: E402
from eopf.dask_utils import dask_helpers  # noqa: E402
from eopf.dask_utils.dask_cluster_type import ClusterType  # noqa: E402
from eopf.dask_utils.dask_context_manager import DaskContext  # noqa: E402
from eopf.dask_utils.dask_context_utils import init_from_eo_configuration  # noqa: E402
from eopf.exceptions.errors import (  # noqa: E402
    DaskClusterNotFound,
    DaskClusterTimeout,
...
@pytest.mark.unit
def test_gateway_mock(caplog):
    caplog.set_level(logging.INFO, logger="eopf.dask_utils.dask_context_manager")
    dask_gateway_options = pytest.importorskip("dask_gateway.options")
    # Create mock cluster
    ...
    mock_gateway.cluster_options.return_value = dask_gateway_options.Options(
        dask_gateway_options.String("image", default="worker:latest", label="Image"),
        dask_gateway_options.Integer("worker_memory", default=4, min=1, max=32, label="Memory"),
    )
        ...
        with mock.patch("dask_gateway.Gateway", return_value=mock_gateway):
            with mock.patch(
                ...
                with DaskContext(
                    ...
                    print(f"DASK Context : {ctx}")
                assert (
                    "Available options on gateway: "
                    "[{'name': 'image', 'value': 'worker:latest'}, {'name': 'worker_memory', 'value': 4}]"
                    in caplog.text
                )
...
@pytest.mark.unit
def test_dashboard_address_warns_when_bokeh_is_missing(caplog):
    client = FakeClient()
    caplog.set_level(logging.INFO, logger="eopf.dask_utils.dask_context_manager")

    with mock.patch("eopf.dask_utils.dask_context_manager.Client", return_value=client):
        with mock.patch("eopf.dask_utils.dask_context_manager.find_spec", return_value=None):
            with DaskContext(
                cluster_type=ClusterType.ADDRESS,
                client_config={"address": "tcp://scheduler:8786"},
            ):
    ...
    assert "Dask dashboard requested but 'bokeh' is not installed." in caplog.text
    assert "eopf[visualize]" in caplog.text
    assert "Dask dashboard address:" not in caplog.text


@pytest.mark.unit
def test_dashboard_address_is_logged_when_bokeh_is_installed(caplog):
    client = FakeClient()
    caplog.set_level(logging.INFO, logger="eopf.dask_utils.dask_context_manager")

    with mock.patch("eopf.dask_utils.dask_context_manager.Client", return_value=client):
        with mock.patch("eopf.dask_utils.dask_context_manager.find_spec", return_value=object()):
            with DaskContext(
                cluster_type=ClusterType.ADDRESS,
                client_config={"address": "tcp://scheduler:8786"},
            ):
    ...
    assert "Dask dashboard address:" in caplog.text
    assert "eopf[visualize]" not in caplog.text
