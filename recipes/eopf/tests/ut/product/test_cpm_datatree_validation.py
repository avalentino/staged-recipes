...
@pytest.mark.unit
def test_product_structure_top_level_group_with_dataset_is_not_empty():
    measurements = xr.DataArray(
        np.ones((2, 2)),
        dims=("y", "x"),
        name="b01",
        attrs={"long_name": "Band 01", "short_name": "b01", "dtype": "float64"},
    ).to_dataset()
    product = DataTree.from_dict({"measurements": measurements}, name="product_name")
    product.attrs["stac_discovery"] = {}
    product.attrs["other_metadata"] = {}
    anomalies = []

    check_eoproduct_validity(product, anomalies, None)

    assert all("Group /measurements cannot be empty" not in anomaly.description for anomaly in anomalies)
...
def test_product_to_model(product, product_model):
    ...
    assert new_model.model_version == "1.0.0"
    assert "model_version" in new_model.model_dump(exclude_unset=True)
