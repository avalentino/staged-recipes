...
def test_fast_zarr_write_by_subdatatree_matches_normal_write(tmp_path):
    ...
    write_datatree(product, subdatatree_path, engine="fast_zarr_subdatatree", zarr_format=2)
...
@pytest.mark.unit
def test_fast_zarr_experimental_warnings_are_logged(caplog):
    caplog.set_level("WARNING", logger="eopf.store.fast_zarr_writer")

    EODataTreeFastZarrWriter._warn_experimental_fast_zarr()
    EODataTreeFastZarrWriter._warn_experimental_subdatatree()

    assert "Experimental feature: fast_zarr is a narrow performance writer" in caplog.text
    assert "Experimental feature: fast_zarr_subdatatree writes one DataTree node at a time" in caplog.text
    assert "local filesystem Zarr targets" in caplog.text


@pytest.mark.unit
def test_fast_zarr_subdatatree_writer_flushes_each_dask_node(tmp_path, monkeypatch):
    import dask.array as da
    ...
    write_datatree(product, tmp_path / "fast-subdatatree-dask.zarr", engine="fast_zarr_subdatatree")

    assert store_calls == [1, 1]


@pytest.mark.unit
def test_fast_zarr_subdatatree_writer_rejects_compute_false(tmp_path):
...
@pytest.mark.unit
def test_fast_zarr_subdatatree_writer_rejects_direct_s3_target():
    with pytest.raises(NotImplementedError, match="direct S3 targets are not supported"):
        write_datatree(
            _fast_zarr_reference_product(),
            "s3://bucket/fast-subdatatree.zarr",
            engine="fast_zarr_subdatatree",
            zarr_format=2,
        )
...
def test_fast_zarr_matches_cpm_zarr_for_zarr_format(tmp_path, zarr_format):
    ...
    write_datatree(product, cpm_zarr_path, engine="cpm_zarr", zarr_format=zarr_format, consolidated=(zarr_format == 2))
    write_datatree(product, fast_zarr_path, engine="fast_zarr", zarr_format=zarr_format, consolidated=(zarr_format == 2))

    xr.testing.assert_identical(
        reader.open_datatree(cpm_zarr_path, consolidated=(zarr_format == 2)),
        reader.open_datatree(fast_zarr_path, consolidated=(zarr_format == 2)),
    )
    if zarr_format == 2:
...
def test_fast_zarr_matches_cpm_zarr_with_encoding(tmp_path, zarr_format):
    ...
    write_datatree(product, cpm_zarr_path, engine="cpm_zarr", zarr_format=zarr_format, encoding=encoding, consolidated=(zarr_format == 2))
    write_datatree(product, fast_zarr_path, engine="fast_zarr", zarr_format=zarr_format, encoding=encoding, consolidated=(zarr_format == 2))
