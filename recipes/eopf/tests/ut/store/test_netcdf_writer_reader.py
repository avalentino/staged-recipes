...
@pytest.mark.unit
def test_netcdf_writer_forwards_backend_engine_to_xarray(monkeypatch, fake_quality_datatree: DataTree, tmp_path) -> None:
    captured = {}

    def fake_to_netcdf(self, filename_or_obj, **kwargs):
        captured["dtree"] = self
        captured["filename_or_obj"] = filename_or_obj
        captured["kwargs"] = kwargs
        return "netcdf-result"

    monkeypatch.setattr(DataTree, "to_netcdf", fake_to_netcdf)

    result = write_datatree(
        fake_quality_datatree,
        tmp_path / "output.nc",
        engine="cpm_netcdf",
        backend_engine="h5netcdf",
        mode="w",
    )

    assert result == "netcdf-result"
    assert captured["filename_or_obj"] == tmp_path / "output.nc"
    assert captured["kwargs"]["engine"] == "h5netcdf"
