...
@pytest.mark.unit
def test_zarr_reader_does_not_log_storage_secrets(monkeypatch, caplog):
    monkeypatch.setattr(
        "eopf.store.zarr_reader.xr.open_datatree",
        lambda filename_or_obj, **kwargs: DataTree(name="product"),
    )

    with caplog.at_level("INFO", logger="eopf.store.zarr_reader"):
        EODataTreeZarrReader().open_datatree(
            filename_or_obj="product.zarr",
            use_obstore=False,
            storage_options={
                "key": "clear-key",
                "secret": "clear-secret",
    ...
    assert "clear-secret" not in caplog.text
    assert "clear-key" not in caplog.text
...
def zarr_product_written_on_disk(setup_and_cleanup_files, tmp_path: Path):
    ...
    root = zarr.open_group(file_name, mode="w",zarr_format=3)
    ...
    ).to_zarr(store=f"{file_name}/coordinates/grid", mode="a", consolidated=False)
    xarray.Dataset(
        {
            "radiance": (("rows", "columns"), np.zeros((2, 3))),
            "orphan": (("depths", "length"), np.zeros((2, 3))),
        },
    ).to_zarr(store=f"{file_name}/coordinates/tie_point", mode="a", consolidated=False)
    ...
    ).to_zarr(store=f"{file_name}/measurements/geo_position/altitude", consolidated=False, mode="a")
    xarray.Dataset(
        {
            "polarian": xarray.DataArray(
                [[1, 2], [3, 4]],
    ...
    ).to_zarr(store=f"{file_name}/measurements/geo_position/latitude", consolidated=False, mode="a")
    xarray.Dataset(
        {
            "polarian": xarray.DataArray(
                [[6, 7, 8], [2, 1, -6]],
    ...
    ).to_zarr(store=f"{file_name}/measurements/geo_position/longitude", consolidated=False, mode="a")
