...
from eopf.store.zarr_writer import EODataTreeSubDataTreeZarrWriter, EODataTreeZarrWriter
...
def test_zarr_write(OUTPUT_DIR, fake_quality_datatree):
    ...
    EODataTreeZarrWriter().write(dtree=fake_quality_datatree, filename_or_obj=output_file, consolidated = False)
...
def test_zarr_write_by_subdatatree_rebases_path_encoding():
    ...
    write_kwargs = EODataTreeSubDataTreeZarrWriter()._get_node_dataset_write_kwargs(
...
def test_zarr_write_by_subdatatree_matches_normal_write(tmp_path):
    ...
    subdatatree_writer = EODataTreeSubDataTreeZarrWriter()
    ...
    subdatatree_writer.write(product, subdatatree_path, zarr_format=2)

    xr.testing.assert_identical(
        reader.open_datatree(normal_path, consolidated=True),
        reader.open_datatree(subdatatree_path, consolidated=True),
    )


@pytest.mark.unit
def test_zarr_subdatatree_writer_rejects_compute_false(tmp_path, caplog):
    caplog.set_level("WARNING", logger="eopf.store.xarray_zarr_writer")

    with pytest.raises(NotImplementedError, match="cpm_zarr_subdatatree does not support compute=False"):
        EODataTreeSubDataTreeZarrWriter().write(
    ...
    assert "Experimental feature: cpm_zarr_subdatatree writes one DataTree node at a time" in caplog.text
    assert "local filesystem Zarr targets" in caplog.text


@pytest.mark.unit
def test_zarr_subdatatree_writer_rejects_direct_s3_target():
