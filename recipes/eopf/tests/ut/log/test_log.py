...
from eopf.logging import (
    configure_logging_from_directory,
    configure_logging_from_file,
    install_password_filter,
    reset_logging,
    setup_basic_logging,
)
...
@pytest.mark.unit
def test_install_password_filter_masks_existing_named_logger_handlers(tmp_path: Path):
    reset_logging()
    log_file = tmp_path / "masked.log"
    logger = logging.getLogger("eopf.test.masked")
    ...
    install_password_filter()
    logger.info("service password=clear-secret")
    handler.close()

    assert "password=****" in log_file.read_text()
