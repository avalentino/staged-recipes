...
@pytest.mark.parametrize(
    ...
    [
        ...
        "s3://dpr-cpm-input/cpm/unit_test_data",
    ],
)
def test_resolve_storage_options_from_secrets_warns_with_secret_and_s3_path(
    tmp_path: Path,
    ...
    secret_file.write_text(
        json.dumps(
            {
                "secret_bindings": {binding: "test_data"},
                "test_data": {
                    "key": "resolved-key",
                    "secret": "resolved-secret",
                },
    ...
    try:
        ...
        s3_path = "s3://dpr-cpm-input/cpm/unit_test_data/product"
        with caplog.at_level(logging.WARNING, logger="eopf.common.file_utils"):
            result = resolve_storage_options_from_secrets(s3_path, None)

        assert result == {"key": "resolved-key", "secret": "resolved-secret"}
        msg = caplog.text
        assert "secret 'test_data'" in msg
        assert s3_path in msg
        assert "s3_cloud.client_kwargs.endpoint_url" in msg
        assert "s3_cloud.client_kwargs.region_name" in msg
    finally:
        SecretsManager.clear()
...
def test_missing_top_level_parameters(cfg, missing, caplog: pytest.LogCaptureFixture):
    with caplog.at_level(logging.WARNING, logger="eopf.common.file_utils"):
        result = format_s3_cloud_config(cfg)
...
def test_missing_client_kwargs_parameters(cfg, missing, caplog: pytest.LogCaptureFixture):
    with caplog.at_level(logging.WARNING, logger="eopf.common.file_utils"):
        result = format_s3_cloud_config(cfg)
...
def test_original_input_is_not_modified():
    cfg = {
        "key": "k",
        "secret": "s",
        "client_kwargs": {"endpoint_url": "url", "region_name": "r"},
    }
