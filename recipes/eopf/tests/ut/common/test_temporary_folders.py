...
@pytest.mark.unit
def test_local_temp_dir_creates_missing_base_folder(OUTPUT_DIR):
    base_folder = AnyPath.cast(os.path.join(OUTPUT_DIR, "NoTHere"))
    EOConfiguration()["temporary__folder"] = str(base_folder)

    temp_dir = EOTemporaryFolder().get().copy()

    assert base_folder.isdir()
    assert temp_dir.exists()
    EOTemporaryFolder.clear()
    base_folder.rm(recursive=True)


@pytest.mark.unit
def test_temp_dir_error_cases():
